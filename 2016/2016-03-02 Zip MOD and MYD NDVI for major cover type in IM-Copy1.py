...
ax1.plot(df_RC['NDVI'][period].index.astype(datetime),y_RC_HANTS[0]/10000., '#ffff64', lw=2,label='Low',
...
ax1.plot(df_RC['NDVI'][period].index.astype(datetime),df_RC['NDVI'][period]/10000.,color='#ffff64',
         linestyle='None',marker='o', markersize=3,label='RC')
L_RC = mlines.Line2D([], [], color='#ffff64', lw=2, label='Rainfed croplands',
                     path_effects=[path_effects.Stroke(linewidth=3, foreground='black'),
                       path_effects.Normal()])
ax1.set_ylim(-0.1,1)
ax1.axhline(color='k')
...
ax2.plot(df_MC['NDVI'][period].index.astype(datetime),y_MC_HANTS[0]/10000., '#dcf064', lw=2, label='Low',
        path_effects=[path_effects.Stroke(linewidth=3, foreground='black'),
                       path_effects.Normal()])         
ax2.plot(df_MC['NDVI'][period].index.astype(datetime),df_MC['NDVI'][period]/10000., color='#dcf064',
         linestyle='None',marker='o', markersize=3,label='MC')
L_MC = mlines.Line2D([], [], color='#dcf064', lw=2, label='Mosaic cropland / vegetation',
        path_effects=[path_effects.Stroke(linewidth=3, foreground='black'),
                       path_effects.Normal()])
...
ax3.plot(df_MV['NDVI'][period].index.astype(datetime),y_MV_HANTS[0]/10000., '#cdcd66', lw=2,label='Low',
        path_effects=[path_effects.Stroke(linewidth=3, foreground='black'),
                       path_effects.Normal()])
         
ax3.plot(df_MV['NDVI'][period].index.astype(datetime),df_MV['NDVI'][period]/10000., color='#cdcd66', 
         linestyle='None', marker='o', markersize=3,label='MV')
L_MV = mlines.Line2D([], [], color='#cdcd66', lw=2, label='Mosaic vegetation / cropland',
        path_effects=[path_effects.Stroke(linewidth=3, foreground='black'),
                       path_effects.Normal()])
...
ax5.plot(df_ON['NDVI'][period].index.astype(datetime),y_ON_HANTS[0]/10000., '#286400', lw=2,label='Low',
        path_effects=[path_effects.Stroke(linewidth=3, foreground='black'),
                       path_effects.Normal()])
         
ax5.plot(df_ON['NDVI'][period].index.astype(datetime),df_ON['NDVI'][period]/10000.,color='#286400', 
         linestyle='None',marker='o', markersize=3,label='ON')
L_ON = mlines.Line2D([], [], color='#286400', lw=2, label='Open needle-leaved deciduous or evergreen forest',
        path_effects=[path_effects.Stroke(linewidth=3, foreground='black'),
                       path_effects.Normal()])
...
ax6.plot(df_CO['NDVI'][period].index.astype(datetime),y_CO_HANTS[0]/10000., '#ffb432', lw=2,label='Low',
        path_effects=[path_effects.Stroke(linewidth=3, foreground='black'),
                       path_effects.Normal()])
         
ax6.plot(df_CO['NDVI'][period].index.astype(datetime),df_CO['NDVI'][period]/10000.,color='#ffb432', 
         linestyle='None', marker='o', markersize=3, label='CO')
L_CO = mlines.Line2D([], [], color='#ffb432', lw=2, label='Closed to open herbaceous vegetation',
        path_effects=[path_effects.Stroke(linewidth=3, foreground='black'),
                       path_effects.Normal()])
...
ax7.plot(df_SV['NDVI'][period].index.astype(datetime),y_SV_HANTS[0]/10000., '#ffebaf', lw=2, label='Low',
        path_effects=[path_effects.Stroke(linewidth=3, foreground='black'),
                       path_effects.Normal()])
         
ax7.plot(df_SV['NDVI'][period].index.astype(datetime),df_SV['NDVI'][period]/10000., color='#ffebaf',
         linestyle='None', marker='o', markersize=3, label='SV')
L_SV = mlines.Line2D([], [], color='#ffebaf', lw=2, label='Sparse vegetation',
        path_effects=[path_effects.Stroke(linewidth=3, foreground='black'),
                       path_effects.Normal()])
...
ax8.plot(df_BA['NDVI'][period].index.astype(datetime),y_BA_HANTS[0]/10000., '#fff5d7', lw=2, label='Low',
        path_effects=[path_effects.Stroke(linewidth=3, foreground='black'),
                       path_effects.Normal()])
         
ax8.plot(df_BA['NDVI'][period].index.astype(datetime),df_BA['NDVI'][period]/10000.,color='#fff5d7', 
         linestyle='None', marker='o', markersize=3,label='BA')
L_BA = mlines.Line2D([], [], color='#fff5d7', lw=2, label='Bare areas',
        path_effects=[path_effects.Stroke(linewidth=3, foreground='black'),
                       path_effects.Normal()])
                     
ax8.set_ylim(-0.1,1)
ax8.set_xlim('2007-12-31','2008-12')
ax8.axhline(color='k')
ax8.xaxis_date()
ax8.yaxis.grid()
...
leg = ax4.legend(handles=[L_RC,L_MC,L_MV,L_ON,L_CO,L_SV,L_BA], loc=2, bbox_to_anchor=(-0.05, 1.00))
leg.get_frame().set_linewidth(0.0)
leg.get_frame().set_facecolor('w')
ax4.axis('off')
#f.legend()#handles=[l1,l2,l3,l8,l5,l6,l7],loc='upper right')#, loc = (axbox.x0 + x_value, axbox.y0 + y_value))
f.tight_layout()
f.savefig(r'D:\Downloads\ZhouJie@Mattijn//hants_2008.png', dpi=200)
