...
def _NETAI_CAL(date,spl_arr):
    ...
    field['COVERAGEID']='AET_RADI'#'trmm_3b42_coverage_1'
    ...
    #logging.info('Current position:',cur_pos)
    ...
    for d in datelist:
        ...
        field['COVERAGEID']='AET_RADI'#'radi_et'#'trmm_3b42_coverage_1'
        ...
        #logging.info(cube_arr[0])#print d

    ##calculate the regional VCI
    cube_arr_ma=ma.masked_equal(numpy.asarray(cube_arr),-3000)
    NETAI=(cube_arr_ma[cur_pos,:,:]-numpy.mean(cube_arr_ma,0))*1.0/(numpy.amax(cube_arr_ma,0)-numpy.amin(cube_arr_ma,0))
    
    NETAI += 1
    NETAI *= 1000
    NETAI //= (2000 - 0 + 1) / 255. # instead of 256 to make space for zero values
    NETAI = NETAI.astype(numpy.uint8)
    NETAI += 1 # So 0 values are reserved for mask
    ...
    fmt = 'GTiff'
    NETAIFileName = 'NETAI'+cur_date.strftime("%Y%m%d")+'.tif'
    driver = gdal.GetDriverByName(fmt)
    dst_dataset = driver.Create(NETAIFileName, ncol, nrow, nband, gdal.GDT_Byte)
    dst_dataset.SetGeoTransform(geotransform)
    dst_dataset.SetProjection(spatialreference)
    dst_dataset.GetRasterBand(1).WriteArray(NETAI)
    dst_dataset = None
    return NETAIFileName, cube_arr_ma, cur_pos

# <codecell>

date='2010-06-30'
spl_arr=[111,30.5,118,35.8]#[70,30,80,50]

# <codecell>

NETAIfn, cube_arr_ma, cur_pos=_NETAI_CAL(date,spl_arr)

# <codecell>

plt.imshow(cube_arr_ma[0])

# <codecell>

(cube_arr_ma[cur_pos,:,:]-numpy.mean(cube_arr_ma,0))*1.0

# <codecell>

(numpy.amax(cube_arr_ma,0)-numpy.amin(cube_arr_ma,0))

# <codecell>

(cube_arr_ma[cur_pos,:,:]-numpy.mean(cube_arr_ma,0))*1.0/(numpy.amax(cube_arr_ma,0)-numpy.amin(cube_arr_ma,0))
...
class Process(WPSProcess):
    ...
    def execute(self):
        ...
        NETAIfn=_NETAI_CAL(date,spl_arr)
        self.dataOut.setValue( NETAIfn )
