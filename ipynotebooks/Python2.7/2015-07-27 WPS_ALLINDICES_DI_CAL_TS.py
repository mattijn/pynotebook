...
def REQUEST_DATA_XML(lon_center=5.6943, lat_center=53.1533, pix_offset=0, coverageID = 'modis_13c1_cov'):
    ...
    full_url = "http://159.226.117.95:8080/rasdaman/ows/wcs2?service=WCS&version=2.0.1&request=GetCoverage&coverageId="+coverageID+"&subset=Long("+Long1+","+Long2+")&subset=Lat("+Lat1+","+Lat2+")"
    ...
    return ts_reshape, dates,str_date,end_date

# <codecell>

def COMPUTE_NVAI(ts_reshape, dates, str_date, end_date, pyhants=0, ann_freq=6, outliers_rj='None', 
...
def COMPUTE_NTAI(ts_reshape, dates, str_date, end_date, pyhants=0, ann_freq=6, outliers_rj='None', 
                 from_date='2010-01-01', to_date='2012-01-01'):
    """
...
def COMPUTE_VCI(ts_reshape, dates, str_date, end_date, pyhants=0, ann_freq=6, outliers_rj='None', 
                 from_date='2010-01-01', to_date='2012-01-01'):
    """
    ...
    if pyhants == 0:
        ...
        vci.name = 'vci'
        vci_sel = nvai.ix[from_date:to_date]
...
def COMPUTE_TCI(ts_reshape, dates, str_date, end_date, pyhants=0, ann_freq=6, outliers_rj='None', 
                 from_date='2010-01-01', to_date='2012-01-01'):
    """
...
class Process(WPSProcess):
    ...
    def __init__(self):
        ...
        WPSProcess.__init__(self,
            ...
            title="Compute ALL DI TIMESERIES",
            abstract="""Module to compute ALL DI TimeSeries based on NDVI and LST data""",
            version = "1.0",
            storeSupported = True,
        ...
        self.tciOut = self.addComplexOutput(identifier  = "tci_ts", 
                                        title       = "TCI Timeseries",
                                        formats     = [{'mimeType':'text/xml'}]) #xml/application ||application/json   

        self.vhiOut = self.addComplexOutput(identifier  = "vhi_ts", 
                                        title       = "VHI Timeseries",
                                        formats     = [{'mimeType':'text/xml'}]) #xml/application ||application/json           
        ...
        self.ntaiOut = self.addComplexOutput(identifier  = "ntai_ts", 
                                        title       = "NTAI Timeseries",
                                        formats     = [{'mimeType':'text/xml'}]) #xml/application ||application/json   
    ...
    def execute(self):
        ...
        vci_json = COMPUTE_VCI(ndvi,dates,str_date,end_date)
        nvai_json = COMPUTE_NVAI(ndvi,dates,str_date,end_date)

        # Request LST data and compute TCI and compute TCI
        lst, dates, str_date, end_date = REQUEST_DATA_XML(coverageID='modis_11c2_cov')
        tci_json = COMPUTE_TCI(lst,dates,str_date,end_date)
        ntai_json = COMPUTE_NTAI(lst,dates,str_date,end_date)

        # Load VCI and TCI data and compute VHI
        vhi_json = COMPUTE_VHI(vci_json, tci_json)
        
        # 3. Save to out
        self.vciOut.setValue( vci_json )        
        self.tciOut.setValue( tci_json )        
        self.vhiOut.setValue( vhi_json )        
        self.nvaiOut.setValue( nvai_json )        
        self.ntaiOut.setValue( ntai_json )
        return
