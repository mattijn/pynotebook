...
for i in range(0,308):
    iso = start+timedelta(i * 16)
    dates_iso.append(iso)
    
    ansi = startt+(i * 16)
    dates_ansi.append(ansi)
dates_iso = dates_iso[65:247]    
...
for k,d in enumerate(dates_ansi[65:66]):
    print d
    field={}
    field['SERVICE']='WCS'
    field['VERSION']='2.0.1'
    ...
    fmt = 'GTiff'
    directory = r'D:\Downloads\Mattijn@Shiva\2015-10-20_NDVI_Rasdaman//'
    nvaiFileName = 'NDVI_'+dates_iso[k].strftime("%Y%m%d")+'.tif'
