...
class Process(WPSProcess):
     def __init__(self):
         ...
         WPSProcess.__init__(self,
              identifier       = "trechterkut", 
              title            = "***---*** Anomaly Computation ***---***",
              version          = "1",
              storeSupported   = "true",
              statusSupported  = "false",
              abstract         = "This service provide access to compute the absolute, NVAI or standardized anomaly including exponentially-weighted moving average with a choice to base it on HANTS")
     ...
     def execute(self):  
         ...
         window_size = int(self.windo.getValue())
         ...
         if based_on_hants == 1:
             ...
             ts_reshape = HANTS(sample_count=time, inputs=ts_reshape_trmm/100, frequencies_considered_count=frequencies,  outliers_to_reject=outliers)
         ...
         #df = pd.DataFrame(ts, dates)#ts_reshape.flatten(), dates)
