...
class Process(WPSProcess):
	...
	#self.randIn = self.addLiteralInput(identifier="random",
	#	    title = "The randomness")
        ...
        self.tmsfOut = self.addLiteralOutput(identifier = "tms_folder",
                title="Output TMS folder")
	...
	path = '/var/www/html/wpsoutputs/'
	TMS_folder = 'TMS_'+str(randomnumber).zfill(6)
	out_url = path+TMS_folder
	cmd = 'python '+'/usr/bin/gdal2tiles.py '+'-w '+'openlayers '+'-z '+'3-5 '+vcifn+' '+out_url

	logging.info(vcifn)
	logging.info(TMS_folder)
	logging.info(cmd)
	...
	self.tmsfOut.setValue( TMS_folder )
