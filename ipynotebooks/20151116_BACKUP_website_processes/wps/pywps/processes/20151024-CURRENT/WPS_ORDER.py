...
#     NTAI += 1 # So 0 values are reserved for mask
    ...
    dst_dataset = driver.Create(directory, ncol, nrow, nband, gdal.GDT_Float64)
...
def email(info_order, url_link):
    msg = MIMEMultipart()
    msg['From'] = 'East-Asian Drought Monitor <m.vanhoek@radi.ac.cn>'
    msg['To'] = info_order[-1]
    msg['Subject'] = 'Data request East-Asian Drought monitoring platform'
    message =  """
    Dear %(name)s,
    ...
    Earth Observation for Water Cycle (EOWater)
...
class Process(WPSProcess):
    ...
    def execute(self):
        ...
        bbox_order = map(float, self.bboxIn.getValue().split(','))
        array_order = self.arrayIn.getValue().split(',')
        info_order = self.infoIn.getValue().split(',')
        ...
        uuid_in = get_a_uuid()
        server_path = '/var/www/html/wpsoutputs/wpsorders/'+uuid_in+'/'
        client_path = 'http://159.226.117.95//wpsoutputs/wpsorders/'+uuid_in
        ...
        logging.info(info_order[2]) 
        logging.info(info_order[-1]) 
